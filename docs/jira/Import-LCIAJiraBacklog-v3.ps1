...
Write-Host "`nTipos disponibles devueltos por Jira:"
foreach ($type in ($issueTypes | Sort-Object hierarchyLevel, name)) {
    Write-Host ("  - {0} | id={1} | hierarchyLevel={2} | subtask={3}" -f $type.name, $type.id, $type.hierarchyLevel, $type.subtask)
}

function Resolve-ProjectIssueType {
    param(
        [Parameter(Mandatory = $true)][string]$CsvType
    )

    $aliases = @{
        "Epic"  = @("Epic", "Épica", "Epica")
        "Story" = @("Story", "Historia")
        "Task"  = @("Task", "Tarea")
    }

    $matches = @(
        $issueTypes | Where-Object {
            $candidate = $_.name
            @($aliases[$CsvType] | Where-Object {
                [string]::Equals($_, $candidate, [System.StringComparison]::OrdinalIgnoreCase)
            }).Count -gt 0
        }
    )
    ...
    # Fallback seguro para Epic: en la jerarquía estándar es el único nivel 1.
    if ($CsvType -eq "Epic") {
        $levelOne = @($issueTypes | Where-Object {
            -not $_.subtask -and [int]$_.hierarchyLevel -eq 1
        })
        if ($levelOne.Count -eq 1) {
            return $levelOne[0]
        }
    }

    if ($matches.Count -gt 1) {
        Fail "Hay varios tipos candidatos para '$CsvType': $($matches.name -join ', ')."
    }

    Fail "No se pudo resolver '$CsvType'. Jira devolvió: $($issueTypes.name -join ', ')."
}

$resolvedIssueTypes = @{}
foreach ($requiredType in @("Epic", "Story", "Task")) {
    $t = Resolve-ProjectIssueType $requiredType
    $resolvedIssueTypes[$requiredType] = $t
    Write-Host ("CSV {0} -> Jira '{1}' (id={2}, hierarchyLevel={3})" -f $requiredType, $t.name, $t.id, $t.hierarchyLevel)
}

if ([int]$resolvedIssueTypes["Epic"].hierarchyLevel -ne 1) {
    Fail "El tipo resuelto para Epic no está en hierarchyLevel=1."
}
foreach ($baseType in @("Story", "Task")) {
    if ([int]$resolvedIssueTypes[$baseType].hierarchyLevel -ne 0) {
        Fail "El tipo resuelto para $baseType no está en hierarchyLevel=0."
    }
}

$projectStatuses = @(Invoke-Jira -Method "GET" -Path "/rest/api/3/project/$ProjectKey/statuses")
foreach ($requiredType in @("Epic", "Story", "Task")) {
    $jiraType = $resolvedIssueTypes[$requiredType]
    $statusGroup = $projectStatuses |
        Where-Object { $_.id -eq $jiraType.id -or $_.name -eq $jiraType.name } |
        Select-Object -First 1

    if ($null -eq $statusGroup) {
        Fail "No se pudo leer el workflow/statuses para CSV '$requiredType' -> Jira '$($jiraType.name)'."
    }

    $backlog = @($statusGroup.statuses | Where-Object {
        [string]::Equals($_.name, "Backlog", [System.StringComparison]::OrdinalIgnoreCase)
    })

    if ($backlog.Count -eq 0) {
        Fail "'$($jiraType.name)' no tiene el estado Backlog en su workflow."
    }
}
Write-Host "Workflow: los tres tipos resueltos contienen Backlog."

# Qué campos admite el endpoint de creación para cada tipo
$createFieldsByType = @{}
foreach ($requiredType in @("Epic", "Story", "Task")) {
    $type = $resolvedIssueTypes[$requiredType]
    ...
    foreach ($mandatory in @("summary", "labels")) {
        if ($mandatory -notin $fieldIds) {
            Fail "El campo '$mandatory' no está disponible al crear '$requiredType' (Jira '$($type.name)'). Es obligatorio para una importación segura e idempotente."
        }
    }
}
...
function Create-One($Row) {
    ...
    $type = $resolvedIssueTypes[$typeName]
